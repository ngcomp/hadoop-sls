...
public class ResourceSchedulerWrapper  extends AbstractYarnScheduler<SchedulerApplicationAttempt, SchedulerNode> implements ResourceScheduler, Configurable {
    ...
    private Map<ApplicationId, String> appQueueMap = new ConcurrentHashMap<ApplicationId, String>();
    ...
    //RAMP
    private BufferedWriter nodeMetricsLog;
    private StringBuilder nodeMetricsStringBuilder;
            ...
            public void run() {
                try {
                    if (metricsLogBW != null) {
                        ...
                        //RAMP
                        nodeMetricsStringBuilder.deleteCharAt(nodeMetricsStringBuilder.length() - 1);
//                        nodeMetricsLog.write(nodeMetricsStringBuilder.toString());
//                        nodeMetricsLog.close();

                        String chart = new String(Files.readAllBytes(Paths.get("/tmp/chart.html")));
                        chart = chart.replace("<<CHART_DATA>>", nodeMetricsStringBuilder.toString());
                        nodeMetricsLog.write(chart);
                        nodeMetricsLog.close();
                    }
                    if (web != null) {
                        web.stop();
    ...
    public Allocation allocate(ApplicationAttemptId attemptId, List<ResourceRequest> resourceRequests, List<ContainerId> containerIds, List<String> strings, List<String> strings2) {
        if (metricsON) {
            ...
            try {
                allocation = scheduler.allocate(attemptId, resourceRequests, containerIds, strings, strings2);
                return allocation;
            } finally {
                context.stop();
    ...
    public void handle(SchedulerEvent schedulerEvent) {
        ...
        try {
            ...
            if (schedulerEvent.getType() == SchedulerEventType.NODE_UPDATE && schedulerEvent instanceof NodeUpdateSchedulerEvent) {
                ...
                //RAMP
                nodeMetricsStringBuilder.append('[').append("new Date(" + System.currentTimeMillis()+")").append(',');
...
//                    REBOOTED;
                    ...
                    int memory = this.rm.getResourceScheduler().getNodeReport(node.getNodeID()).getAvailableResource().getMemory();

                    nodeMetricsStringBuilder.append(node.getTotalCapability().getMemory() - memory).append(',');
//                    int vCores = this.rm.getResourceScheduler().getNodeReport(node.getNodeID()).getAvailableResource().getVirtualCores();
//                    nodeMetricsStringBuilder.append(name).append(",").append(memory).append(',').append(vCores);
                }
                nodeMetricsStringBuilder.deleteCharAt(nodeMetricsStringBuilder.length() - 1);
                nodeMetricsStringBuilder.append("],");
    ...
    private void initMetrics() throws Exception {
        ...
        Class schedulerMetricsClass = schedulerMetricsType == null ? defaultSchedulerMetricsMap.get(scheduler.getClass()) : Class.forName(schedulerMetricsType);
    ...
    class MetricsLogRunnable implements Runnable {
        ...
        public MetricsLogRunnable() {
            try {
                ...
                metricsLogBW.write("[");
                //RAMP
                nodeMetricsLog = new BufferedWriter(new FileWriter(metricsOutputDir + "/node_metric.html"));
                nodeMetricsLog.write("[");
                nodeMetricsStringBuilder = new StringBuilder();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
            ...
            if (running) {
                ...
                try {
                    if (firstLine) {
                        ...
                        //RAMP: first line will contain all node names..
                        if(nodeMetricsStringBuilder == null){
                            nodeMetricsStringBuilder = new StringBuilder();
                        }
                        nodeMetricsStringBuilder.append("['Time',");
                        for (RMNode node : rm.getRMContext().getRMNodes().values()) {
//                            if (node.getState() == NodeState.RUNNING) {
                                nodeMetricsStringBuilder.append("'" + node.getHostName() + "',");
//                            }
                        }
                        nodeMetricsStringBuilder.deleteCharAt(nodeMetricsStringBuilder.length() -1);
                        nodeMetricsStringBuilder.append("],");
                        System.out.println(nodeMetricsStringBuilder.toString());
